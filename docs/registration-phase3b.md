...
The management page shows runner, entry, payment/refund, declaration eligibility and race-number state without provider/database identifiers. It can continue an unpaid or expired payment, request an individual refund, make limited non-identity amendments and initiate a transfer. Transfer requires a complete new adult runner record, invalidates the previous token immediately, resets the declaration to pending and sends new registration-specific links. The replacement management token is delivered to the new runner and is not returned to the previous runner's browser.

The organiser dashboard keeps ordinary phone/club correction separate from an explicit **Transfer entry** action. Organiser transfer preserves the existing place and payment/refund history, replaces entrant ownership, revokes old management/declaration links, resets declaration compliance and records an audit event. The normal runner transfer cutoff remains enforced. A post-cutoff organiser transfer requires a separately selected override, and the use of that override is recorded; it does not silently relax the runner rule.

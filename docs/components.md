...
`components/navbar/navbar.html` contains the logo and the four primary destinations: Results, Route, Info and the aligned Enter action. Recce and Kit Swap remain dedicated pages but appear in the same Information tab row as Travel, Kit list, Race Report and statistics. The compact mobile menu uses two columns. Every top-level page embeds the primary navigation in an iframe with class `nav-iframe`; the component posts its open or closed height to the parent page and `script.js` safely applies that height.

...
Phase 3B adds a disabled-by-default integration layer for Stripe-hosted test Checkout, raw-body signed webhooks, idempotent reconciliation, full sandbox refunds and controlled ACS Email delivery. It does not alter the production artifact. See `registration-phase3b.md` for the payment/email state machines, scheduled-work boundary and remaining provider proof gates.
